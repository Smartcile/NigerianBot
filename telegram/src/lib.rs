//! Telegram surface: a thin bot over the same core as the Discord bot and web
//! GUI. Reads/writes the shared Postgres (download queue, identity); background
//! work is done by the worker. Exposed as a library for the all-in-one binary.
...
/// Start the Telegram bot (long polling). Returns early if no token is set, so
/// the all-in-one binary can boot without Telegram configured.
pub async fn run() -> anyhow::Result<()> {
    if common::config::optional("TELEGRAM_BOT_TOKEN").is_none() {
        warn!("TELEGRAM_BOT_TOKEN not set — telegram surface disabled");
        return Ok(());
    }
