//! Discord surface: connect to Discord, register the slash commands, route
//! interactions, run voice, and drive the in-process scheduler.
//!
//! Exposed as a library so the all-in-one `nigerianbot` binary can run it
//! in-process; the `nigerian-bot` binary runs it standalone.
...
/// Connect to Discord and run until the process stops.
